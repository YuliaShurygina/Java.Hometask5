...
public class waveAlgoritm {
    ...
    public static boolean isInList(final ArrayList<int[]> list, final int[] candidate){ // метод определения находиться ли в эррейлисте массив с двумя координатами
    ...
    public static void findPath(int [][] matrix, int x, int y, ArrayList<int []> path, ArrayList<int []> barriers){ // находим обратный путь
        ...
        if (matrix[x][y] == 1){
            ...
            for (int i = -1; i <= 1; i++ ){
                for (int j = -1; j <= 1; j++ ){
                    ...
                    if ( !isInList(barriers, coord) && x1 >= 0 && y1>= 0 && x1 < matrix.length && y1 < matrix[x1].length && matrix[x1][y1] == matrix[x][y] - 1){
